...
struct brain_stats_t {
    ...
    vex::fontType current_font = vex::fontType::mono20;
...
char *prefix_sd(const char *name) {
    char *new_name = (char *) malloc(strlen(name) + 3 + 1);
    char *end = strncpy(new_name, "sd/", 4);
    strncpy(new_name + 3, name, strlen(name) + 1);
...
int calc_string_width(const char *str) {
    ...
    return length_sum;
        ...
        bool setup()
        {
            ...
            // setup sd card
            sd_card_in = std::filesystem::exists("sd/");
